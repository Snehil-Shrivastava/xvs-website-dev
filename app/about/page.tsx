        ...
        (context) => {
          ...
          gsap.to(svgRef.current, {
            ...
            y: () => {
              ...
              // @ts-ignore
              const richTxtRect = richTxtRef.current.getBoundingClientRect();

              // 1. Find the bottom of the rich text relative to the pinned container
              const richTxtBottom = richTxtRect.bottom - containerRect.top; // 566 - 160

              // 2. Calculate the height of the SVG *at its final scale*
              const svgFinalHeight = svgRect.height * finalScale; // 98 * 0.8

              // 3. The final top position of the SVG should be the rich text bottom minus the SVG's final height
              const finalTopPosition = richTxtBottom - svgFinalHeight; // 406 - 78.4

              // 4. The required translation is the difference between where it needs to be and where it currently is
              const currentTopPosition = svgRect.top - containerRect.top; // 160 - 160

              console.log("finalScale", finalScale);

              console.log("containerRect", containerRect);
              console.log("svgRect", svgRect);
              console.log("richTxtRect", richTxtRect);
              console.log("richTxtBottom", richTxtBottom);
              console.log("svgFinalHeight", svgFinalHeight);
              console.log("finalTopPosition", finalTopPosition);
              console.log("currentTopPosition", currentTopPosition);

              return finalTopPosition - currentTopPosition;
            },
          });
        }
      );
  ...
  return (
    <section className={`${isModalOpen ? "invisible" : ""}`}>
      <div className="relative">
        ...
        <div
          ref={containerRef}
          className={`absolute left-1/2 -translate-x-1/2 max-sm:top-40 pointer-events-none max-sm:h-24 max-sm:w-21 sm:max-lg:top-56.5 sm:max-lg:w-29 sm:max-lg:h-33.75 lg:max-xl:top-80 lg:max-2xl:w-51 lg:max-2xl:h-59.25 xl:max-2xl:top-95 min-[1536px]:max-[1906px]:w-80 min-[1536px]:max-[1906px]:h-100 min-[1536px]:max-[1906px]:top-100 min-[1906px]:w-110 min-[1906px]:h-130 min-[1906px]:top-120 border border-blue-500`}
        >
          <div className="relative h-full">
            ...
            <div
              ref={svgRef}
              className={`absolute ${styles.svgRefDebug} border border-amber-600`}
            >
              <svg
                // ref={svgRef}
                xmlns="http://www.w3.org/2000/svg"
      ...
      <div
        // ref={richTxtRef}
        className="relative pt-[72px] max-[768px]:pt-0 border border-red-500"
        // style={{ opacity: 0 }}
      >
        {/* <RichText /> */}
        <svg
          ...
          viewBox="0 0 217 172"
          fill="none"
          className="w-auto max-sm:h-20 sm:max-lg:h-35.5 lg:max-xl:h-43 min-[1906px]:h-70 mx-auto scale-[1] translate-x-[5px] border border-emerald-500"
        >
          <path
